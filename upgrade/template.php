...
<div id="login-box">
	...
	<div style="width:283px;height:82px;float:left;background: url({$theme}/skins/images/loginbox7.png);"><div class="info">To update the script, you need to enter username and password.</div></div>
...
if ($_POST['action'] == "dologin")
{
	...
	if ($db->num_rows() > 0){
	        ...
	        $is_logged = TRUE;
	} else $result="<font color=red>Incorrect username or password!</font>";
...
if(!is_writable(ENGINE_DIR.'/data/config.php')){
	msgbox("info","Information", "Set right to write in file 'engine/data/config.php' CHMOD 666");
}

if(!is_writable(ENGINE_DIR.'/data/dbconfig.php')){
	msgbox("info","Information", "Set right to write in file 'engine/data/dbconfig.php' CHMOD 666");
}
...
<table width="100%">
    <tr>
        <td style="padding:2px;"><font color="red"><b>Attention:</b></font> Before you proceed to update the script and database, make sure that you have created and saved in a full backup script files and databases. The update procedure makes permanent changes to the database structure, the abolition of which in future will be impossible to return to a previous state database, it will be possible only by restoring backups of the database. Also during the upgrade script runs heavy queries against the database, the implementation of which may require a long time, so update is recommended during the minimum load on the server. For large sites with a large number of publications, it is recommended to carry out the update on your computer.
		<br /><br />
		The current version of the script: <b>{$version_id}</b>, update will be made ??incrementally to version: <b>{$dle_version}</b>
		<br /><br />
</td>
    </tr>
    <tr>
        <td style="padding:2px;"><input class=buttons type=submit value=" Proceed >> "></td>
    </tr>
</table>
</td>
        <td background="{$theme}/skins/images/tl_rb.gif"><img src="{$theme}/skins/images/tl_rb.gif" width="6" height="1" border="0"></td>
